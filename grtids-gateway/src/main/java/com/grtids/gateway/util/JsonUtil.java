package com.grtids.gateway.util;
...
//		http.GET(url);
//		java.io.InputStream bstream = http.getInputStream();
...
//		return sbs.toString();
	...
	public static Map getMap4Url(String url) {
		try {
			String jsonString = getContent(url);
			return getMap4Json(jsonString);
		} catch (Exception e) {
			log.error("文件上传异常：", e);
		}
		return null;
	 ...
	 * @return
	 */
	public static List getListFormJson(String jsonString, Class pojoClass) {
	 ...
	 * @return
	 */
	public static Map getMapFormJson(String jsonString) {
		JSONObject jsonObject = JSONObject.fromObject(jsonString);
		Iterator keyIter = jsonObject.keys();
		String key;
		Object value;
